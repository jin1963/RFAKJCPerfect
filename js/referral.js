...
// ✅ สมัคร Referrer อัตโนมัติจาก URL ถ้ายังไม่มี
export async function autoSetReferrerFromURL() {
  try {
    if (!window.account || !window.stakingContract) return;
    ...
    if (currentRef && currentRef !== "0x0000000000000000000000000000000000000000") {
      console.log("มี referrer อยู่แล้ว:", currentRef);
      return; // มี referrer แล้วไม่ต้องตั้งใหม่
    }

    const urlParams = new URLSearchParams(window.location.search);
    const ref = urlParams.get('ref');
    if (ref && window.web3.utils.isAddress(ref) && ref.toLowerCase() !== window.account.toLowerCase()) {
      console.log("กำลังตั้งค่า referrer อัตโนมัติ:", ref);
      await window.stakingContract.methods.setReferrer(ref).send({ from: window.account });
      alert(`✅ สมัคร Referrer อัตโนมัติเรียบร้อย: ${ref}`);
      updateUI();
    }
  } catch (e) {
    console.error("autoSetReferrerFromURL error:", e);
  }
}

// สมัคร Referrer ผ่าน smart contract (manual)
